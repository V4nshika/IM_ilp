...
def seq_cut_ilp(G, log, sup=1.0):
        """
        Calculates the sequence cut using ILP.
        W_forward_cache removed to ensure local correctness.
        """
        ...
        # Always Calculate from scratch for the current recursion level
...
def seq_cut_ilp_linearized(G, log, sup=1.0):
    ...
    for i, j in f1_indices:
        if i != start_idx:
            x_i_term = x[i] if i in non_terminal else 0
            model.addConstr(f1[i, j] <= (n ) * (1 - x_i_term), name=f"flow1_cap_i_{i}_{j}")
        if j != end_idx:
            x_j_term = x[j] if j in non_terminal else 0
            model.addConstr(f1[i, j] <= (n ) * (1 - x_j_term), name=f"flow1_cap_j_{i}_{j}")
    ...
    for i, j in f2_indices:
        if i != start_idx:
            x_i_term = x[i] if i in non_terminal else 1
            model.addConstr(f2[i, j] <= (n ) * x_i_term, name=f"flow2_cap_i_{i}_{j}")
        if j != end_idx:
            x_j_term = x[j] if j in non_terminal else 1
            model.addConstr(f2[i, j] <= (n ) * x_j_term, name=f"flow2_cap_j_{i}_{j}")
