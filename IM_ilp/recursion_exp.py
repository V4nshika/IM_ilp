...
def recursion_full(log, timing_data=None, depth=0, max_depth=20, sup=1.0):
    ...
    if len(all_activities) == 1:
        ...
        # Restore the simple mathematical logic from recursion_full_old
        ...
        # Calculate c+ cost for empty traces proportion (τ-xor check)
    ...
    cut_results = {
        'exc_tau':  {'Sigma_1': None, 'Sigma_2': None, 'cost': float('inf'), 'n': 0, 'time': 0, 'success': False},
        'loop_tau': {'Sigma_1': None, 'Sigma_2': None, 'cost': float('inf'), 'n': 0, 'time': 0, 'success': False},
    ...
    # Add tau cuts first as they are faster to check
    cut_functions = [
        ('exc', xor_cut_ilp),      
        ('seq', seq_cut_ilp),      
        ('par', par_cut_ilp),      
        ('loop', loop_cut_ilp),
        ('exc_tau', xor_cut_tau),    # Check tau cuts LAST
    ...
    for op_name, cut_func in cut_functions:
        ...
        try:
            ...
            if op_name == 'loop_tau':
                Sigma_1, Sigma_2, total_cost, nodes = cut_func(G, sup)
            # Call the appropriate cut function
            if op_name == 'exc':
                Sigma_1, Sigma_2, total_cost, nodes = cut_func(G, sup)
            elif op_name == 'seq':
            ...
            # 1. A binary cut is valid if both partitions are non-empty
            valid_binary_cut = (not is_tau_cut) and (Sigma_1 and Sigma_2)
            
            # 2. A tau cut is valid if Sigma_1 is non-empty
            if is_tau_cut and Sigma_1:
                valid_tau_cut = True
                if Sigma_2 is None: # Standardize for the split function
                    Sigma_2 = set()
            else:
                valid_tau_cut = False
            
            # Check if the cut is valid (binary OR tau) and has a valid cost
            if (total_cost is not None and total_cost < float('inf') 
                and (valid_binary_cut or valid_tau_cut)):
                
                # Standardize Sigma_2 for tau cuts (from None to set())
                if Sigma_2 is None:
                    Sigma_2 = set()
...
def apply(log, sup =1):
    import time
    start = time.time()
    process_tree = recursion_full(log, sup)
    end = time.time()
