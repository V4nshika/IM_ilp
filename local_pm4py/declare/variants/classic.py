...
from local_pm4py.declare.templates import *
import pandas as pd
...
def __rule_atmost_one_column(act: str) -> Tuple[str, str]:
    return (ATMOST_ONE, act)


def __rule_init_column(act: str) -> Tuple[str, str]:
    return (INIT, act)

def __rule_end_column(act: str) -> Tuple[str, str]:
    return (END, act)
...
def atleast_one_template_step1(rules: Dict[Union[Tuple[str, str], Tuple[str, str, str]], int], trace: Collection[str],
                               activities: Set[str], act_counter: TCounter[str], act_idxs: Dict[str, List[int]],
                               allowed_templates: Collection[str]):
    if ATMOST_ONE in allowed_templates:
        for act in activities:
            if act in act_counter:
                if act_counter[act] == 1:
                    rules[__rule_atmost_one_column(act)] = 1
                else:
                    rules[__rule_atmost_one_column(act)] = -1
            else:
                rules[__rule_atmost_one_column(act)] = 1
...
def precedence_template_step1(rules: Dict[Union[Tuple[str, str], Tuple[str, str, str]], int], trace: Collection[str],
    ...
    if PRECEDENCE in allowed_templates:
        for act2 in act_counter:
            for act in activities:
                if act2 != act:
                    if act not in act_counter:
                        rules[__rule_precedence(act, act2)] = -1
                    else:
                        # if (act == 'A2') and (act2 == 'A21'):
                        #     print('oo')
                        # if act_idxs[act2][0] < act_idxs[act][0]:
                        #     rules[__rule_precedence(act2, act)] = 1
                        if act_idxs[act][0] < act_idxs[act2][0]:
                            rules[__rule_precedence(act, act2)] = 1
                        else:
                            rules[__rule_precedence(act, act2)] = -1
...
def atleast_one_template_step2(dataframe: pd.DataFrame, columns: Collection[str], activities: Set[str],
                               allowed_templates: Collection[str]) -> pd.DataFrame:
    if ATMOST_ONE in allowed_templates:
        for act in activities:
            if __rule_atmost_one_column(act) not in columns:
                dataframe[__rule_atmost_one_column(act)] = [0] * len(dataframe)
    return dataframe
...
def nonsuccession_template_step2(dataframe: pd.DataFrame, columns: Collection[str], activities: Set[str],
                            allowed_templates: Collection[str]) -> pd.DataFrame:
    if NONSUCCESSION in allowed_templates:
        for act in activities:
            for act2 in activities:
                if act2 != act:
                    if __rule_non_succession(act, act2) not in columns:
                        dataframe[__rule_non_succession(act, act2)] = [0] * len(dataframe)
    return dataframe
...
def form_rules_dataframe(log: Union[EventLog, pd.DataFrame],
    ...
    for trace, occs in vars.items():
        ...
        atleast_one_template_step1(rules, trace, activities, act_counter, act_idxs, allowed_templates)
        ...
        nonsuccession_template_step1(rules, trace, activities, act_counter, act_idxs, allowed_templates)
    ...
    dataframe = nonsuccession_template_step2(dataframe, columns, activities, allowed_templates)
